//gcc -Wall SnakeOrigin.c -o snake -lncurses -DNCURSES_STATIC
 ...
 *2. Внести изменения в исполняемый файл, чтобы в стандартном режиме змейка не погибала 
 при самопересечении. Для этого необходимо найти вызов функции isCrush() и поменять вызов н
 а нужное нам возвращаемое значение 0
 ...
 * 2. Modify executable so that in standard mode snake doesn't die from self-intersection. 
 * For this, find isCrush() function call and change it to return value 0
